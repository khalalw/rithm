...
describe("#linearSearchRecursive", function() {
  it("should find the index of a value in an array", function() {
    expect(linearSearchRecursive([1, 2, 3, 4], 4, 0)).to.equal(3);
    expect(linearSearchRecursive([1, 2], 1, 0)).to.equal(0);
    expect(linearSearchRecursive([1, 2, 3, 4, 5, 6, 7], 6, 0)).to.equal(5);
  });
  it("should return -1 if the value is not found", function() {
    expect(linearSearchRecursive([1, 2, 3, 4], 0, 0)).to.equal(-1);
    expect(linearSearchRecursive([1, 3, 4], 2, 0)).to.equal(-1);
    expect(linearSearchRecursive([1, 2], 11, 0)).to.equal(-1);
  });
});
